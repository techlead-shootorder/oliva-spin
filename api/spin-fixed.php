...
try {
    ...
    try {
        ...
        // Method 1: Try to get tier-specific code
        try {
        ...
        // Method 2: Fallback to unique generated code
        if (!$assignedCode) {
            try {
                $assignedCode = generateUniqueCouponCode($winningCoupon['coupon_code'], $recordedId);
                $codeSource = 'generated_unique';
            } catch (Exception $e) {
                error_log("Unique code generation failed: " . $e->getMessage());
        ...
        if (!$assignedCode) {
            $assignedCode = $winningCoupon['coupon_code'] . '-' . strtoupper(substr(md5($recordedId . time()), 0, 6));
            $codeSource = 'simple_fallback';
        }
    ...
    error_log("Critical spin error for recordedId $recordedId: " . $e->getMessage());
